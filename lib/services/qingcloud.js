...
(function() {
  ...
  _receiveWebhook = function(arg) {
    var body, integration, message, payload, resource, resource_json, rules, rules_json, text, title;
    ...
    if (resource_json.resource_name !== '') {
    ...
    text = [];
    _.forIn(rules_json, function(rule) {
      if (rule.alarm_policy_rule_name !== '') {
        return text.push("RULE_ID: " + rule.alarm_policy_rule_id + " RULE_NAME: " + rule.alarm_policy_rule_name + " STATUS: " + rule.status);
      } else {
        return text.push("RULE_ID: " + rule.alarm_policy_rule_id + " STATUS: " + rule.status);
      }
    });
    text = text.join('\n');
    message = {
  ...
  module.exports = function() {
    ...
    this.iconUrl = util["static"]('images/icons/qingcloud@2x.png');
    this._fields.push({
      key: 'token',
      type: 'text',
